...
void Text::updateTextTexture() {
    ...
    if (renderTexture.create({static_cast<unsigned>(textRect.width),
        ...
        renderTexture.draw(*m_pText.get());

        renderTexture.display();
        auto texture = new sf::Texture(renderTexture.getTexture());
        setTexture(texture);
    } else {
        LOG_CRITICAL("Background of text did not create!");
    }
}
[[maybe_unused]] void Text::setText(const sf::Text& text) {
    m_pText = std::make_unique<sf::Text>(text);
    updateTextTexture();
}
...
[[maybe_unused]] void Text::setColor(sf::Color color) {
    m_pText->setFillColor(color);
    updateTextTexture();
}
[[maybe_unused]] void Text::setFont(const sf::Font& font) {
    m_pText->setFont(font);
    updateTextTexture();
}
[[maybe_unused]] void Text::setCharacterSize(unsigned int size) {
