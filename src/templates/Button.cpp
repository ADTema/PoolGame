...
[[maybe_unused]] void Button::setText(const std::string& string) {
    m_pText->setString(sf::String::fromUtf8(string.begin(), string.end()));
    updateTextureBackground();
}
[[maybe_unused]] void Button::setText(sf::Text text) {
    m_pText = std::make_unique<sf::Text>(std::move(text));
    m_font = *m_pText->getFont();
    updateTextureBackground();
}
[[maybe_unused]] void Button::setFont(sf::Font font) {
...
[[maybe_unused]] void Button::setBackgroundColor(sf::Color color) {
    m_backgroundColor = color;
    updateTextureBackground();
}
...
[[maybe_unused]] void Button::setCharacterSize(unsigned int size) {
    m_pText->setCharacterSize(size);
    updateTextureBackground();
}
[[maybe_unused]] void Button::setTextColor(sf::Color color) {
...
[[maybe_unused]] void Button::setMouseClickAction(std::function<void()> action) {
    m_actionFunctions[Mouse::Click] = std::move(action);
}
[[maybe_unused]] void Button::setMouseDownAction(std::function<void()> action) {
    m_actionFunctions[Mouse::Down] = std::move(action);
}
[[maybe_unused]] void Button::setMouseUpAction(std::function<void()> action) {
    m_actionFunctions[Mouse::Up] = std::move(action);
}
[[maybe_unused]] void Button::setMouseOverAction(std::function<void()> action) {
    m_actionFunctions[Mouse::Over] = std::move(action);
}
[[maybe_unused]] void Button::setMouseOutAction(std::function<void()> action) {
